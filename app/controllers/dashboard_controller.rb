...
class DashboardController < ApplicationController
  ...
  def index
    ...
    dependentes = Dependente.select("select pessoas.id, dependentes.dependente_id, pessoas.nome, pessoas.data_nascimento").joins(:pessoa).where("dependentes.pessoa_id IN (?) AND pessoas.data_nascimento <> 'NULL'", irmaos.map(&:pessoa_id))
