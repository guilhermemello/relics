class ComboController < ApplicationController
  def cidades_por_uf
    @cidades = Cidade.select("DISTINCT cidades.id, cidades.nome").where("cidades.estado_id = ?", params[:estado_id]).all
  ...
  def bairros_por_cidade
    @bairros = Bairro.select("DISTINCT bairros.id, bairros.nome").where("bairros.cidade_id = ?", params[:cidade_id]).all
